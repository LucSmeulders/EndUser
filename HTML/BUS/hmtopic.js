...
Copyright (c) 2015-2022 by Tim Green
All Rights Reserved */
...
function aniScroll(target, mode, noAnimate, tjump) {
	...
	// alert("Aniscroll animation speed: " + speed);
	...
	setTimeout(function(timer) {
		...
		$(sTarget).scrollTo($(target), speed, { offset: -16, axis: "y",
			...
			if (!tjump) flashTarget($(target[0]).parent(),3,200);
			}
		});	
		},timer);
}
...
if (document.location.hash) {
  var theTargetID = document.location.hash.toString().replace(/\#/, "");
  var theTarget;
...
if ($("a[name]").length > 0 && $("a.dropdown-toggle").length > 0) {
	...
	if ("onhashchange" in window) {
		var pageInit=true;
		var lastInit = new Date().getTime(), currentInit;
		$(window).on('hashchange', function(event) {
		event.preventDefault();
		currentInit=new Date().getTime() - lastInit;
		lastInit = new Date().getTime();
		if (pageInit) {pageInit = false} else {
			if (currentInit > 3000) {
			toggleJump(true);
			}
			}
